...
def create_ecvaluation_table(eval_url, cookie, dept_list, year_list, course_num_dict,\
                      instructor_num_dict, url_list, overwrite = True):
    course_eval_table = []
    ins_comm_table = []     ##store the comments
    ins_eval_table = []     ##store the evaluation score
    ins_score_table = []
    past_dept = ''
            ...
            if counter != 0:
                for ins_id in ins_list:
                    ins_eval_table.append([course_id, year, class_section, quarter, \
                                       ins_id, temp_total_score/counter])
            else:
                for ins_id in ins_list:
                    ins_eval_table.append([course_id, year, class_section, quarter,\
                                       ins_id, 'No score'])
        ...
        if i == 0 and check_first == True:
            ...
            for instructor_id in ins_list:
                ins_score_table.append([course_id, year, \
                    class_section, quarter, instructor_id, score_term, avg_score])
...
#    dept_list = ['STAT']
#    dept_list = ['ARAB', 'STAT', 'ECON']
    dept_list = ['AASR']
#    dept_list = ['ECON']
    year_list = ['2005']
    year_list = ['2015']
#    dept_list = dept_list[166:]
    course_num_dict = {}
    if course_compile:
...
#    driver.get("https://evaluations.uchicago.edu")
#    cookie = relog_in(driver, eval_url, user_name, password)
    d, e, f = create_ecvaluation_table(eval_url, cookie, dept_list, year_list, course_num_dict,\
                      instructor_num_dict, url_list, overwrite = True)
